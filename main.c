...
AutoStartConnections()
{
    ...
    for (i = 0; i < o.num_configs; i++)
    {
        wprintf(L"config_file = %s\n", o.conn[i].config_file);
...
ShowSettingsDialog()
{
  ...
  psp[page_number].dwFlags = PSP_DLGINDIRECT;
  psp[page_number].hInstance = o.hInstance;
  psp[page_number].pResource = LocalizedDialogResource(ID_DLG_NETEASE);
  psp[page_number].pfnDlgProc = NeteaseDialogFunc;
  psp[page_number].lParam = 0;
  psp[page_number].pfnCallback = NULL;
  ++page_number;
