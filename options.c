...
{
    // GoString name = {o.exe_path, _countof(o.exe_path)};
    // SayHello(name);
    // char* _char = SayBye("test");
    // int iLength = MultiByteToWideChar (CP_ACP, 0, _char, strlen (_char) + 1, NULL, 0);  
    // MultiByteToWideChar (CP_ACP, 0, _char, strlen (_char) + 1, o.config_dir, iLength);
    SetDlgItemText (hdlg, ID_EDT_CONFIG_DIR, o.config_dir);
    SetDlgItemText (hdlg, ID_EDT_CONFIG_EXT, o.ext_string);
    SetDlgItemText (hdlg, ID_EDT_LOG_DIR, o.log_dir);
...
INT_PTR CALLBACK
NeteaseDialogFunc(HWND hwndDlg, UINT msg, UNUSED WPARAM wParam, LPARAM lParam) {
    LPPSHNOTIFY psn;
    switch(msg) {
        case WM_INITDIALOG:
            // load account & password
            // CharToTchar(SayBye("hello"), o.corp_account);
            // CharToTchar(SayBye("daniel"), o.corp_password);
            CharToTchar(GetNeteaseCorpAccount(), o.corp_account);
            CharToTchar(GetNeteaseCorpPassword(), o.corp_password);

            SetDlgItemText (hwndDlg, ID_EDT_CORP_ACCOUNT, o.corp_account);
            SetDlgItemText (hwndDlg, ID_EDT_CORP_PASSWORD, o.corp_password);
            break;
        case WM_NOTIFY:
            psn = (LPPSHNOTIFY) lParam;
            if (psn->hdr.code == (UINT) PSN_APPLY) {
                // save account & password
                GetDlgItemText (hwndDlg, ID_EDT_CORP_ACCOUNT, o.corp_account, _countof(o.corp_account));
                GetDlgItemText (hwndDlg, ID_EDT_CORP_PASSWORD, o.corp_password, _countof(o.corp_password));
                GoString account = {o.corp_account, _tcslen(o.corp_account) * 2};
                GoString pass = {o.corp_password, _tcslen(o.corp_password) * 2};
                // printf("count: %d\n", _tcslen(o.corp_account));
                // SayHello(account);
                SaveNeteaseCorpInfo(account, pass);
                return TRUE;
            }
            break;
    }
...
CharToTchar(char* _char, TCHAR* tchar) {
    int iLength = MultiByteToWideChar (CP_ACP, 0, _char, strlen (_char) + 1, NULL, 0);  
